...
Template.commandCenter.helpers({
  totalTrainers: function () {
    totalTrainers = Meteor.users.find({ roles: 'trainer' }).count();

    return totalTrainers;
  },

  totalClients: function () {
    totalClients = Meteor.users.find({ roles: 'client' }).count();

    return totalClients;
  },

  totalUsers: function () {
    totalUsers = Meteor.users.find({ _id: { $ne: Meteor.userId() } }).count();
